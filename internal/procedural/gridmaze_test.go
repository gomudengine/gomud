...
func TestGridMaze_Properties(t *testing.T) {
	maze := NewGridMaze()
	maze.rand.Seed(12345) // Deterministic for testing

	rooms := maze.Generate2D(10, 8)
	...
	for x := 0; x < 10; x++ {
		for y := 0; y < 8; y++ {
			...
			if rx != x || ry != y || rz != z {
				t.Errorf("Room position mismatch: expected (%d,%d,%d), got (%d,%d,%d)", x, y, z, rx, ry, rz)
			}

			// Test connections are bidirectional
			connections := room.GetConnections()
			for _, conn := range connections {
				if !conn.IsConnectedTo(room) {
					t.Error("Connection is not bidirectional")
				}
			}
			...
			// Count critical path rooms
			if room.GetStep() > 0 {
				criticalPathRooms++
			}

			// Test distance from start is reasonable
			dist := room.GetDistanceFromStart()
			if dist < 0 {
				t.Errorf("Negative distance from start: %d", dist)
			}
		}
	}

	t.Logf("Maze statistics: %d rooms, %d dead ends, %d critical path rooms", roomsFound, deadEnds, criticalPathRooms)

	// Verify we have some rooms
	if roomsFound == 0 {
		t.Error("No rooms found in maze")
	}

	// Verify critical path length matches what we found
	criticalPath := maze.GetCriticalPath()
	if len(criticalPath) != criticalPathRooms {
		t.Errorf("Critical path length mismatch: expected %d, got %d", criticalPathRooms, len(criticalPath))
	}
}
