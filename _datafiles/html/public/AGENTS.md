...
- A CSS variable `--background-image` pointing to `web_bg.png` via
  `{{ .ASSET_BASE }}`, which is computed server-side from `WebCDNLocation`
...
that the computed `.ASSET_BASE` is respected.
...
with `{{ .ASSET_BASE }}`. To add a new window module, add
...
```html
<!-- Left dock -->
<script src="{{ .ASSET_BASE }}/static/js/windows/window-example.js"></script>
```
