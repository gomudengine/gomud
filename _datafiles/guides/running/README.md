...
such as `80` or `443`, `make run` will rerun with `sudo` automatically. For
local development, you can avoid that by using `CONFIG_PATH` with an override
file that sets unprivileged ports such as `8080` and `8443`.
...
If you want a built binary to bind privileged ports without running the whole
server as root, grant it `cap_net_bind_service` and then run it directly:

> `sudo setcap 'cap_net_bind_service=+ep' ./GoMudServer`

Or if you have docker installed:

> `docker compose up --build`

On Linux and other Unix-like hosts, if Docker is not directly available to your
current user, `make run-docker` will rerun the Docker command through `sudo`.
On Windows hosts, it uses the normal Docker command path without a `sudo`
fallback.
